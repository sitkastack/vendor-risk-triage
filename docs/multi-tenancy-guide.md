...
3. **Schema migration engine (0.12.0, shipped).** The `migration` package and `vrt migrate` CLI subcommand up-migrate records across the output-contract version chain. The additive hops (1.0.0 through 1.2.0) are version restamps; the 1.2.0-to-1.3.0 hop assigns a `tenant_id` to pre-tenancy records via an explicit, operator-supplied resolver (whole-batch or per-record), optionally constrained to a registry. The engine never defaults a tenant silently. This is what carries pre-tenancy records forward into the tenant-attributed world. See `docs/migration-guide.md`.
