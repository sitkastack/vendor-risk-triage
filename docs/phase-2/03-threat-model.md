...
Mitigation: Multiple layers, none individually sufficient. Schema validation at intake constrains the field structures the LLM sees (per ADR-004). The Phase 1 input contract is parsed structurally and the agent renders submission content into the LLM prompt inside explicit BEGIN_SUBMISSION/END_SUBMISSION delimiters, with system prompt instructions to treat content between the delimiters as data not instructions (agent/agent.py, Phase 3 sub-system 2). Document content from the Phase 3 sub-system 4 ingestion path is wrapped in BEGIN_DOCUMENT/END_DOCUMENT delimiters with the same instruction discipline. Retrieved regulation chunks (Phase 3 sub-system 5, Phase 4 sub-system 5) are wrapped in BEGIN_REGULATION_CONTEXT/END_REGULATION_CONTEXT delimiters. The Output Validator catches structurally invalid responses but cannot catch valid responses that reflect injected reasoning. Phase 4 sub-system 1 (eval/attacks/) ships a 12-attack baseline dataset covering 8 attack categories with measurable pass rates per category and per threat ID; deploying organizations extend the suite with attacks specific to their threat surface. Human review of escalated cases provides the final safety net. Prompt injection cannot be fully eliminated; residual risk is documented below.
...
Mitigation: Zero data retention at the provider level (per ADR-001) prevents cross-session exfiltration. The agent's system prompt (agent/agent.py, hashed into agent_version via SYSTEM_PROMPT_HASH) instructs the LLM to refuse extraction requests appearing in submission content. The Output Validator structurally constrains responses via the PydanticAI-enforced TriageRecord schema (per ADR-007), limiting the channel through which exfiltrated content can return. Phase 4 sub-system 1 (eval/attacks/) includes a schema-manipulation attack category that exercises the structural constraint. The combination reduces but does not eliminate the threat; residual risk documented below.
...
Mitigation: Multiple Phase 4 layers measure hallucination from different angles. Phase 4 sub-system 2 (eval/citations/) verifies every evidence_cited entry deterministically: input_field_reference paths resolve through a JSONPath-lite parser, and chunk_id mentions in reasoning text are checked against the supplied regulation chunks. Unresolved references and unknown chunk_ids are direct hallucination indicators. The same sub-system computes Jaccard token-overlap grounding scores between reasoning text and cited chunk text; scores below a configurable threshold (default 0.15) flag candidate hallucinations. Phase 4 sub-system 4 (eval/judge/) adds semantic grounding: an LLM-as-judge running the CITATION_GROUNDING rubric scores whether the cited chunk actually supports the agent's claim about it, a question token overlap cannot answer. Cross-model judging is recommended (per ADR-016). Phase 3 sub-system 4 (ingestion/) verifies document content_hash against the submission's claimed hash before any LLM call, defeating bait-and-switch attacks where attackers submit a hash for one document and substitute another at inference time (per ADR-010). The output contract requires rationale to be specific and reviewable. Hallucination cannot be fully eliminated in current LLMs; residual risk documented below.
...
Mitigation: Phase 4 sub-system 3 (eval/calibration/) computes Brier score, Expected Calibration Error (ECE), and Maximum Calibration Error (MCE) over the agent's confidence_signal.score values on graded datasets. Calibration drift detection in production compares current calibration metrics against a baseline window; sudden ECE or MCE excursions indicate manipulation or model drift. The scorer supports three correctness dimensions (tier, disposition, both) configurable per audit policy. Per-bin reliability data supports the auditor's question: "your AI says 85% confident; show me that 85% confidence means 85% accuracy." Threshold tuning for HITL routing is institutional configuration; institutions periodically review whether thresholds still serve their risk posture. The institution's program owner retains visibility into the routing distribution through the audit trail; sudden distribution shifts are observable.
...
Mitigation: Phase 4 sub-system 4 (eval/judge/) ships a RATIONALE_COHERENCE rubric that scores whether the agent's reasoning is anchored to specific submission facts rather than to characteristics correlated with protected categories. The reference framework provides the rubric template; deploying organizations adapt it to their vendor categories of concern and add bias-specific rubrics. Phase 4 sub-system 1 (eval/attacks/) supports bias-probing attacks as a custom category. The institution's vendor diversity policy and procurement governance provide upstream controls. Output rationale review by human reviewers catches some bias patterns; the structured rationale field and evidence_cited entries support systematic post-hoc review. Bias cannot be fully eliminated; residual risk documented below.
...
Mitigation: Phase 3 evaluation harness (eval/) supports representative sampling of the current vendor population via the graded dataset format; the harness runs the agent over the dataset and produces accuracy metrics per tier and per disposition. Periodic re-evaluation when the institution's vendor mix shifts materially is the operational pattern. Phase 4 sub-system 3 (eval/calibration/) extends this with per-tier calibration breakdowns showing where the agent is well-calibrated and where it has drifted. The agent_version field captures the FRAMEWORK_VERSION and SYSTEM_PROMPT_HASH at decision time (per ADR-003 and ADR-007); comparison of pre-change and post-change classification rates supports drift detection.
...
Mitigation: ADR-001 selects providers based partly on model versioning transparency. Provider contract terms specify model version pinning where available (Anthropic's API supports model version specification per ADR-001). The agent_version field captures the framework's FRAMEWORK_VERSION (currently 0.6.0) and SYSTEM_PROMPT_HASH (currently 69ef583c6dbe), so any change to the agent's own prompt or behavior surfaces as an agent_version change in the audit trail (per ADR-003 and ADR-007). Provider model version is captured separately in deployment_metadata at the institutional layer. Phase 3 evaluation (eval/) and Phase 4 evaluation depth (eval/attacks/, eval/citations/, eval/calibration/, eval/judge/) re-run on a stable test set when the provider announces material changes; metric excursions identify drift even if the provider has not announced changes.
...
**Prompt injection (T-AI1).** Cannot be fully eliminated with current LLM technology. The reference architecture reduces the surface through layered defenses (delimiter discipline in agent/agent.py, structured output constraints via PydanticAI per ADR-007, the attack suite in eval/attacks/ that measures pass rate per attack category) but does not close it. Residual risk: a sufficiently sophisticated injection produces an attacker-chosen classification despite the defenses. Institutional mitigation: HITL review of escalated cases (and ideally a sample of approved cases) provides the final safety net. Institutions with higher exposure may require human review of all high-risk vendor decisions.

**Hallucination (T-AI4).** Cannot be fully eliminated with current LLM technology. The reference architecture reduces incidence through deterministic citation verification (eval/citations/), semantic grounding via LLM-as-judge (eval/judge/), bait-and-switch defense at the ingestion boundary (per ADR-010), and the output contract's structured rationale and evidence_cited requirements. It does not eliminate hallucination. Residual risk: a triage record contains fabricated content that survives the layered checks. Institutional mitigation: reviewer training to recognize hallucination patterns, sampling-based audit of approved records, regulator engagement when hallucination in a regulatory citation is identified.
...
These artifacts are scoped to a future governance phase, where they integrate with the institution's broader governance discipline and the ISO/IEC 42001 Annex A controls documented in docs/governance/README.md. Phase 4 in this framework's actual roadmap delivered evaluation depth (eval/attacks/, eval/citations/, eval/calibration/, eval/judge/) and retrieval quality rather than formal governance artifacts; the formal threat-to-control matrix and risk register remain on the roadmap for Phase 5 (Operational Hardening) and beyond. The threat model in Phase 2 provides the substantive analysis that the future governance phase formalizes; the formality follows the substance rather than preceding it.

The detection skeletons in detection/ provide the executable interface that Phase 5 implements against. Risk register integration, KRI definition, and risk owner assignment are governance concerns the future governance phase addresses; the threat-and-detection substance lives here.
...
- Phase 3 (Agent + RAG + Ingestion + Eval, shipped): mitigations referenced as "Phase 3" above are implemented in the agent/, ingestion/, retrieval/, and eval/ packages. The agent's system prompt, the BEGIN/END delimiters around submission, document, and regulation content, the bait-and-switch hash verification at the ingestion boundary, and the graded evaluation harness all land in Phase 3.
- Phase 4 (Eval Depth + Retrieval Quality, shipped): the deeper evaluation signals land here. eval/attacks/ measures prompt-injection resistance. eval/citations/ verifies references deterministically and computes token-overlap grounding scores. eval/calibration/ computes Brier, ECE, and MCE. eval/judge/ adds semantic grounding via LLM-as-judge. retrieval/ adds vector and hybrid retrieval as complementary signals to the Phase 3 BM25 baseline.
- Phase 5 (Operational Hardening, upcoming): operational threat monitoring, calibration drift detection across time windows, audit-log shipping format, schema migration patterns, persistent indexes. Detection mechanisms identified in this document are operationalized in Phase 5.
- Phase 6 (Production Polish, upcoming): observability hooks, cost tracking, CI/CD patterns for deployers, performance optimization. The detection signals from the threat model become alerts and dashboards here.

## Status

Phase 2 (Architecture and Threat Model) of the sitkastack Framework, complete as of May 24, 2026. Updated May 26, 2026 to reflect shipped Phase 3 and Phase 4 mitigations: prospective references to "Phase 3 implementation" have been replaced with concrete code references to agent/, ingestion/, retrieval/, and eval/ packages. All five Phase 2 artifacts (problem definition, system architecture, trust boundaries, threat model, and architecture decisions) are published.
