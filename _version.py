...
- 1.0.5 (Determinism attestation). Adds output contract 1.4.0 with a
  ``determinism_attestation`` field on every record. Every triage
  decision now carries a per-record attestation of the producing
  configuration's contract posture: temperature, provider,
  effective_model_id, fallback path, sampling_profile_hash,
  system_prompt_hash (full 64-char SHA-256), corpus_bundle_hash,
  contract_version, and migrated_from. The single boolean a compliance
  lead checks is ``contract_honored``; when true, the framework
  attests reproducibility within an empirically-measured
  per-(provider, model) variance band documented in
  ``docs/determinism-attestation.md``.
  Discovered as the leading framework gap in a five-vendor newsletter
  triage run: Glean's classification varied between tier_1_low and
  tier_2_moderate across runs with identical configuration (3+3
  six-run test against NIST AI RMF and OSFI E-23). The investigation
  isolated the variance to agent non-determinism at tier boundaries
  rather than corpus drift or input ambiguity. The contract
  introduced here makes that variance EXPLICIT and AUDITABLE on every
  record rather than invisible.
  Cross-cutting work in this release:
  (1) Schema bump 1.3.0 -> 1.4.0 with closed enum on FallbackRecord.
  (2) Pydantic model DeterminismAttestation with structurally-required
      keys (null means absent, not missing key; the audit contract).
  (3) Temperature pinning via PydanticAI's model_settings; refuse to
      construct on non-zero temperature unless
      ``allow_nondeterministic_legacy=True``.
  (4) Fallback path produces a FallbackRecord with reason in
      {transient_retry, hard_refusal, circuit_open, operator_pinned,
      cross_provider}. cross_provider always flips
      contract_honored=false because the contract is
      per-(provider, model).
  (5) Migration engine adds the 1.3.0 -> 1.4.0 hop: a lifted record
      gets a "migrated_from" attestation with every data field null
      and contract_honored=false (no contract was in force at
      production time).
  (6) Drift checker surfaces contract_honored flips as SOFT drift; the
      rest of the attestation is ignored as per-deployment instance
      noise.
  (7) triage.completed observability event carries contract_honored,
      effective_temperature, effective_model_id, and fallback_fired
      so sinks can route on contract posture without parsing the
      record body.
  (8) Audit log canonical-bytes computation now expands the attestation
      with all keys present (null included) so two consumers hashing
      the same record agree on bytes regardless of nullable-field
      population.
  (9) ``TriageRecord.model_dump`` overrides exclude_none for the
      attestation specifically: the 1.4.0 schema requires every
      nested key, so the attestation always emits all of them.
  (10) New CI gate: ``scripts/check_system_prompt_hash.py`` asserts the
       framework SYSTEM_PROMPT's SHA-256 matches the committed
       baseline. A prompt edit without a baseline regen fails the
       build, keeping the audit anchor visible in version-control
       history.
  (11) New measurement harness ``scripts/measure_determinism_variance.py``
       runs the agent N times against a fixed submission and emits a
       JSON report of observed variation by field. Populates the
       variance band table in ``docs/determinism-attestation.md``.
  (12) 20 new tests in ``tests/test_agent_determinism.py`` cover
       temperature pinning, refuse / opt-out / legacy paths,
       attestation structure, sampling_profile_hash stability,
       corpus_bundle_hash per-call, fallback enum closure, migration
       semantics, observability payload, and audit log integration.
  (13) Migration tests gain 4 new cases for the 1.3.0 -> 1.4.0 hop
       (attestation populated correctly; idempotent on existing
       attestation; structural validation passes; through-migration
       from 1.0.0).
  Test count goes from 1353 to 1377. Coverage holds at 95% framework
  threshold. SYSTEM_PROMPT_HASH stays 69ef583c6dbe (no prompt edit).
  ``examples/triage-record.example.json`` is bumped to 1.4.0 with a
  populated attestation so the canonical example reflects what the
  framework writes today; ``examples/expected-records/`` stay at 1.3.0
  as historical-shape fixtures used by migration tests. Output
  contract 1.4.0 is additive over 1.3.0: a 1.3.0 record migrates to
  1.4.0 cleanly via ``vrt migrate``; a 1.4.0 record's classification
  fields are byte-identical to what the framework would have produced
  under 1.3.0 (the attestation is the only new structural field).
  Minor bump-feeling work but cataloged as a patch because the new
  field is optional on records declaring versions before 1.4.0; the
  output contract version itself jumped 1.3.0 -> 1.4.0 (a minor bump
  of the contract, not the framework). Future framework versions
  remove ``allow_nondeterministic_legacy`` in 1.1.0; deprecation
  warning is emitted on every use today.
